...
class Updater:
    ...
    def load_help(self):
        ...
        threading.Thread(name="UpdateHTML", target=task_update, daemon=True).start()
    ...
    def check_update(self):
        ...
        def check_ver():
            try:
                ...
                LOGGER.debug(
                    "Check update: Local version=%s, Web version=%s",
                    self.local_version,
                    self.web_version,
                )
        ...
        t = threading.Thread(target=check_ver, name="Check_update", daemon=True)
        t.start()
    ...
    def is_webversion_newer(self) -> bool:
        ...
        try:
            if self.web_version:
                ...
                local_v_int = int(
                    "".join(f"{part:0>4}" for part in self.local_version.split("."))
                )
                web_v_int = int(
                    "".join(f"{part:0>4}" for part in self.web_version.split("."))
                )
    ...
    def download_file(self, fname: str) -> str:
        ...
        with requests.get(
            self.urlbase + fname, stream=True, timeout=self.timeout_dl
        ) as res:
    ...
    def prepare_update(self):
        ...
        if sys.platform == "win32":  # check system support
            ...
            self.update_exception = RuntimeError(
                "Update only supports Windows for now."
            )
            return
        ...
        t = threading.Thread(target=update_task, name="UpdateThread", daemon=True)
        t.start()
