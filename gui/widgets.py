...
class ToggleSwitch(tk.Frame):
    ...
    def __init__(
        ...
        self.img_on = img_on.subsample(
            int(img_on.height() / img_ht), int(img_on.height() / img_ht)
        )
        self.img_off = img_off.subsample(
            int(img_off.height() / img_ht), int(img_off.height() / img_ht)
        )
        self.img_mid = img_mid.subsample(
            int(img_mid.height() / img_ht), int(img_mid.height() / img_ht)
        )
...
class Timer(tk.Frame):
    ...
    def __init__(
        self, master: tk.Frame, height: int, font_size: int = 10, hover_text: str = None
    ):
...
class ToolBar(tk.Frame):
    ...
    def add_button(self, text: str, img_file: str, command) -> tk.Button:
        ...
        img = img.subsample(
            int(img.width() / self.height), int(img.height() / self.height)
        )
        btn = tk.Button(
            self, image=img, width=self.height, height=self.height, command=command
        )
        btn.image = (
            img  # Keep a reference to prevent image from being garbage collected
        )
...
class StatusBar(tk.Frame):
    ...
    def _create_column(self, index: int) -> tk.Frame:
        ...
        label = ttk.Label(
            column_frame, text=f"Column {index + 1}", compound="left"
