...
class MainGUI(tk.Tk):
    ...
    def __init__(self, setting: Settings, bot_manager: BotManager):
        ...
        self.protocol(
            "WM_DELETE_WINDOW", self._on_exit
    ...
    def _create_widgets(self):
        ...
        self.toolbar.add_button(
            self.st.lan().SETTINGS, "settings.png", self._on_btn_settings_clicked
        )
        self.toolbar.add_button(
            self.st.lan().OPEN_LOG_FILE, "log.png", self._on_btn_log_clicked
        )
        self.btn_help = self.toolbar.add_button(
            self.st.lan().HELP, "help.png", self._on_btn_help_clicked
        )
        self.toolbar.add_sep()
        ...
        self.auto_join_level_var = tk.StringVar(
            value=self.st.lan().GAME_LEVELS[self.st.auto_join_level]
        )
        ...
        combo_autojoin_level.bind(
            "<<ComboboxSelected>>", self._on_autojoin_level_selected
        )
        ...
        combo_autojoin_mode.bind(
            "<<ComboboxSelected>>", self._on_autojoin_mode_selected
        )
        # timer
    ...
    def _on_exit(self):
        ...
        if messagebox.askokcancel(
            self.st.lan().EXIT, self.st.lan().EIXT_CONFIRM, parent=self
        ):
            try:
                LOGGER.info("Exiting GUI and program")
                self.status_bar.update_column(
                    2, self.st.lan().EXIT + "ing...", self.icon_yellow
                )
    ...
    def _update_gui_info_inner(self):
        ...
        if not self.bot_manager.browser.is_running():
            if self.bot_manager.get_game_client_type() == GameClientType.PROXY:
                self.btn_start_browser.config(
                    state=tk.DISABLED
                )  # disable when proxy client running
            else:
        ...
        if pending_reaction:
            ai_guide_str, options = mjai_reaction_2_guide(
                pending_reaction, 3, self.st.lan()
            )
            ai_guide_str += "\n"
        ...
        if self.bot_manager.is_running():  # main thread
            self.status_bar.update_column(
                0, self.st.lan().MAIN_THREAD + fps_str, self.icon_green
            )
        else:
            self.status_bar.update_column(
                0, self.st.lan().MAIN_THREAD + fps_str, self.icon_red
            )
    ...
    def _get_status_text_icon(self, gi: GameInfo) -> tuple[str, str]:
        ...
        if self.bot_manager.is_in_game():
            ...
            info_str = (
                self.st.lan().READY_FOR_GAME
                + " - "
                + state_dict.get(self.bot_manager.automation.ui_state, "")
            )
