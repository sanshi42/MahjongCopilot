...
class SettingsWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.ms_url_var, width=std_wid * 3
        )
        ...
        number_entry = ttk.Entry(
            main_frame, textvariable=self.mitm_port_var, width=std_wid
        )
        ...
        _entry = ttk.Entry(
            _frame, textvariable=self.upstream_proxy_var, width=std_wid * 2
        )
        ...
        self.language_var = tk.StringVar(
            value=LAN_OPTIONS[self.st.language].LANGUAGE_NAME
        )
        select_menu = ttk.Combobox(
            main_frame,
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.akagiot_url_var, width=std_wid * 4
        )
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.akagiot_apikey_var, width=std_wid * 4
        )
        string_entry.grid(row=cur_row, column=1, columnspan=3, **args_entry)
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.mjapi_url_var, width=std_wid * 4
        )
        string_entry.grid(row=cur_row, column=1, columnspan=3, **args_entry)
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.mjapi_user_var, width=std_wid
        )
        ...
        string_entry = ttk.Entry(
            main_frame, textvariable=self.mjapi_secret_var, width=std_wid * 4
        )
        string_entry.grid(row=cur_row, column=1, columnspan=3, **args_entry)
        ...
        idle_move_entry = ttk.Checkbutton(
            main_frame,
        ...
        options[0] = "0% (off)"
        self.reply_emoji_var = tk.StringVar(
            value=f"{int(self.st.auto_reply_emoji_rate * 100)}%"
        )
        _combo = ttk.Combobox(
            main_frame,
        ...
        delay_lower_entry = tk.Entry(
            main_frame, textvariable=self.delay_random_lower_var, width=std_wid
        )
        ...
        delay_upper_entry = tk.Entry(
            main_frame, textvariable=self.delay_random_upper_var, width=std_wid
        )
        delay_upper_entry.grid(row=cur_row, column=2, **args_entry)

        # tips :Settings
        cur_row += 1
        label_settings = ttk.Label(
            main_frame, text=self.st.lan().SETTINGS_TIPS, width=std_wid * 4
        )
    ...
    def _on_save(self):
        ...
        if self.mitm_proxinject_updated:
            messagebox.showinfo(
                self.st.lan().SETTINGS,
                self.st.lan().SETTINGS_TIPS,
                parent=self,
                icon="info",
                type="ok",
            )
