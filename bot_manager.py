...
class BotManager:
    ...
    def __init__(self, setting: Settings) -> None:
        ...
        self.mitm_server: mitm.MitmController = (
            mitm.MitmController()
        )  # no domain restrictions for now
        ...
        self.mitm_proxinject_need_update: bool = (
            False  # set this True to update mitm and prox inject in main thread
        )
        self.is_loading_bot: bool = False  # is bot being loaded
        self.main_thread_exception: Exception = (
            None  # Exception that had stopped the main thread
        )
        self.game_exception: Exception = (
            None  # game run time error (but does not break main thread)
        )
    ...
    def _create_bot(self):
        ...
        try:
            ...
            LOGGER.info(
                "Created bot: %s. Supported Modes: %s",
                self.bot.name,
                self.bot.supported_modes,
            )
    ...
    def _create_mitm_and_proxinject(self):
        ...
        if self.st.enable_proxinject:
            mode = mitm.SOCKS5
            LOGGER.debug(
                "Enabling proxyinject requires socks5, and it disables upstream proxy"
            )
        else:
            mode = mitm.HTTP
        ...
        if not res:
            self.main_thread_exception = utils.MitmCertNotInstalled(
                self.mitm_server.cert_file
            )

        if self.st.enable_proxinject:
            self.proxy_injector.start(
                self.st.inject_process_name, "127.0.0.1", self.st.mitm_port
            )
    ...
    def _loop_post_msg(self):
        ...
        self.automation.automate_retry_pending(
            self.game_state
        ...
        if msg.type == mitm.WsType.START:
            ...
            if liqi_method in METHODS_TO_IGNORE:
                ...
                if self.lobby_flow_id is None:  # record first time in lobby
                    ...
                    LOGGER.warning(
                        "Lobby flow exists %s, ignoring new lobby flow %s",
                        self.lobby_flow_id,
                        msg.flow_id,
                    )
                ...
                if self.game_flow_id is None:
                    ...
                    self.game_state.input(
                        liqimsg
                    )  # authGame -> mjai:start_game, no reaction
                    self.game_exception = None
                    self.automation.on_enter_game()
                else:
                    LOGGER.warning(
                        "Game flow %s already started. ignoring new game flow %s",
                        self.game_flow_id,
                        msg.flow_id,
                    )
    ...
    def _update_overlay_guide(self):
        ...
        if reaction:
            ...
            self.browser.overlay_update_guidance(
                guide, self.st.lan().OPTIONS_TITLE, options
            )
        else:
            self.browser.overlay_clear_guidance()
    ...
    def _do_automation(self, reaction: dict):
        ...
        try:
            ...
            LOGGER.error(
                "Failed to automate action for %s: %s",
                reaction["type"],
                e,
                exc_info=True,
            )
...
def mjai_reaction_2_guide(
    reaction: dict, max_options: int = 3, lan_str: LanStr = LanStr()
) -> tuple[str, list]:
    ...
    if re_type == MjaiType.DAHAI:
        ...
        action_str = (
            f"{ActionUnicode.CHI}{lan_str.CHI}{tile_str}({''.join(comsumed_strs)})"
        )
        ...
        dahai_action_str, _dahai_options = mjai_reaction_2_guide(
            reach_dahai_reaction, 0, lan_str
        )
