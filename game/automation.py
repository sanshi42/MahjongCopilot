    ...
    def run_step(self, step: ActionStep):
        ...
        if isinstance(step, ActionStepMove):
            ...
            raise NotImplementedError(
                f"Execution not implemented for step type {type(step)}"
            )
        self.last_exe_time = time.time()

    def start_action_steps(
        self, action_steps: Iterable[ActionStep], game_state: GameState = None
    ):
            ...
            for step in action_steps:
                ...
                if game_state:
                    ...
                    if op_step != new_step and not step.ignore_step_change:
                        LOGGER.debug(
                            "Cancel executing %s due to step change(%d -> %d)",
                            self.name,
                            op_step,
                            new_step,
                        )
        ...
        self._thread = threading.Thread(target=task, name=self.name, daemon=True)
        self._thread.start()
...
class Automation:
    ...
    def __init__(self, browser: GameBrowser, setting: Settings):
        ...
        self.ui_state: UiState = (
            UiState.NOT_RUNNING
        )  # Where game UI is at. initially not running
    ...
    def can_automate(
        self, cancel_on_running: bool = False, limit_state: UiState = None
    ) -> bool:
        ...
        if not self.executor.is_page_normal():  # browser is not running
            return False
        if (
            cancel_on_running and self.is_running_execution()
        ):  # cancel if previous task is running
            return False
        if (
            limit_state and self.ui_state != limit_state
        ):  # cancel if current state != limit_state
            return False
    ...
    def get_delay(self, mjai_action: dict, gi: GameInfo, subtract: float = 0.0):
        ...
        delay = random.uniform(
            self.st.delay_random_lower, self.st.delay_random_upper
        )  # base delay
    ...
    def automate_action(self, mjai_action: dict, game_state: GameState) -> bool:
        ...
        if mjai_type == MjaiType.DAHAI:
            if gi.self_reached:
                ...
                game_state.last_reaction_pending = (
                    False  # cancel pending state so i won't be retried
                )
                return False
        ...
        # "button" action
            ...
            more_steps: list[ActionStep] = self.steps_button_action(
                mjai_action, gi, liqi_operation
            )
        ...
        delay = self.get_delay(
            mjai_action, gi, game_state.last_reaction_time
        )  # initial delay
    ...
    def randomize_action(self, action: dict, gi: GameInfo) -> dict:
        ...
        )  # randomize strength. 0 = no random, 5 = according to probability
        if n == 0:
            return action
        ...
        if mjai_type == MjaiType.DAHAI:
            ...
            for i, cum_prob in enumerate(cumulative_probs):
                if rand_prob < cum_prob:
                    chosen_pai = top_ops_powered[i][
                        0
                    ]  # This is the selected key based on probability
    ...
    def automate_send_emoji(self):
        ...
        if not self.can_automate(True, UiState.IN_GAME):
            return
        if (
            time.time() - self.last_emoji_time < self.st.auto_emoji_intervel
        ):  # prevent spamming
            return
        roll = random.random()
        ...
        steps = [
            ActionStepDelay(random.uniform(1.5, 3.0)),
            ActionStepMove(x * self.scaler, y * self.scaler),
        ]
        ...
        self._task = AutomationTask(
            self.executor, f"SendEmoji{idx}", f"Send emoji {idx}"
        )
    ...
    def automate_idle_mouse_move(self, prob: float):
        ...
        if prob > roll:
            ...
            action_steps += self.steps_move_to_center(False)
            self._task = AutomationTask(
                self.executor, "IdleMove", "Move mouse around in other's turn"
            )
    ...
    def steps_action_dahai(self, mjai_action: dict, gi: GameInfo) -> list[ActionStep]:
        ...
        if tsumogiri:  # tsumogiri: discard right most
            ...
            assert dahai == gi.my_tsumohai, (
                f"tsumogiri but dahai {dahai} != game tsumohai {gi.my_tsumohai}"
            )
            ...
            steps = self.steps_randomized_move(
                Positions.TEHAI_X[idx], Positions.TEHAI_Y
            )
        ...
        if self.st.auto_dahai_drag:
            ...
            steps += self.steps_move_to_center(
                True
    ...
    def _process_oplist_for_kan(self, mstype_from_mjai, op_list: list) -> list:
        ...
        if idx_to_keep is None:
            LOGGER.error(
                "No matching type %s found in op list: %s", mstype_from_mjai, op_list
            )
            return op_list
    ...
    def steps_button_action(
        self, mjai_action: dict, gi: GameInfo, liqi_operation: dict
    ) -> list[ActionStep]:
        """Generate action steps for button actions (chi, pon, kan, etc.)"""
        if (
            "operationList" not in liqi_operation
        ):  # no liqi operations provided - no buttons to click
            return []
        ...
        if the_op is None:  # something's wrong
            LOGGER.error(
                "for mjai %s liqi msg has no op list. Op list: %s", mjai_type, op_list
            )
            return steps
        ...
        # chi / pon / kan: click candidate (choose from options)
            ...
            if len(combs) == 1:  # no need to click. return directly
                ...
                LOGGER.warning(
                    "mjai type %s, but no combination in liqi operation list", mjai_type
                )
                return steps
            for idx, comb in enumerate(combs):
                # for more than one candidate group, click on the matching group
                consumed_liqi = [cvt_ms2mjai(t) for t in comb.split("|")]
                consumed_liqi = sort_mjai_tiles(
                    consumed_liqi
                )  # convert to mjai tile format for comparison
                if (
                    mjai_consumed == consumed_liqi
                ):  # match. This is the combination to click on
    ...
    def steps_randomized_move(self, x: float, y: float) -> list[ActionStep]:
        ...
        steps = []
        if (
            self.st.auto_random_move
        ):  # random moves, within (-0.5, 0.5) x screen size of target
            for _i in range(3):
                ...
                steps.append(
                    ActionStepMove(
                        rx * self.scaler, ry * self.scaler, random.randint(2, 5)
                    )
                )
    ...
    def steps_move_to_center(
        self, ignore_step_change: bool = False
    ) -> list[ActionStep]:
        ...
        move_step = ActionStepMove(
            xmid * self.scaler, ymid * self.scaler, random.randint(2, 5)
        )
    ...
    def automate_end_game(self):
        ...
        self._task = AutomationTask(
            self.executor, END_GAME, "Going back to main menu from game ending"
        )
