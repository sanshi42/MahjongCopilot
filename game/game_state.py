...
class GameState:
    ...
    def _input_inner(self, liqi_msg: dict) -> dict | None:
        ...
        # SyncGame
        if (
            liqi_method == LiqiMethod.syncGame or liqi_method == LiqiMethod.enterGame
        ) and liqi_type == MsgType.RES:
        ...
        # Game Start
            ...
            if liqi_type == MsgType.REQ:
                ...
                raise RuntimeError(
                    f"Unexpected liqi message, method={liqi_method}, type={liqi_type}"
                )

        # Actions
        elif (
            liqi_method == LiqiMethod.ActionPrototype
        ):  # assert all ActionPrototype are notify type
            ...
            if "data" in liqi_data:
                if "operation" in liqi_data["data"]:
                    ...
                    if liqi_data["data"]["operation"]["seat"] != self.seat:
                        LOGGER.warning(
                            "operation seat %s != self.seat %s",
                            liqi_data["data"]["operation"]["seat"],
                            self.seat,
                        )
                    if "operationList" not in liqi_data["data"]["operation"]:
                        LOGGER.warning(
                            "No operation List: %s", liqi_data["data"]["operation"]
                        )
            else:
    ...
    def ms_new_round(self, liqi_data: dict) -> dict:
        ...
        self.kyoku_state.my_tehai = [
            mj_helper.cvt_ms2mjai(tile) for tile in my_tehai_ms
        ]
        ...
        if len(self.kyoku_state.my_tehai) == 14:  # self is East
            ...
            tsumo_msg = {"type": MjaiType.TSUMO, "actor": oya, "pai": "?"}
        else:
    ...
    def ms_action_prototype(self, liqi_data: dict) -> dict:
            ...
            if "doras" in liqi_data_data:
                if len(liqi_data_data["doras"]) > len(self.kyoku_state.doras_ms):
                    self.mjai_pending_input_msgs.append(
                        {
                            "type": MjaiType.DORA,
                            "dora_marker": mj_helper.cvt_ms2mjai(
                                liqi_data_data["doras"][-1]
                            ),
                        }
                    )
        ...
        # LiqiAction.ChiPengGang -> MJAI CHI/PON/DAIMINKAN
            ...
            for idx, fr in enumerate(liqi_data_data["froms"]):
                if fr != actor:
                    ...
                    consumed_mjai.append(
                        mj_helper.cvt_ms2mjai(liqi_data_data["tiles"][idx])
                    )
            ...
            match liqi_data_data["type"]:
                ...
                case _:
                    raise ValueError(
                        f"Unknown ChiPengGang type {liqi_data_data['type']}"
                    )
            return self._react_all(liqi_data_data)
        ...
        # LiqiAction.AnGangAddGang -> MJAI ANKAN / KAKAN
            ...
            match liqi_data_data["type"]:
                case MSGangType.AnGang:
                    ...
                    if (
                        actor == self.seat
                    ):  # update hand info. ankan is after tsumo, so there is tsumohai
                        self.kyoku_state.my_tehai.append(self.kyoku_state.my_tsumohai)
                        self.kyoku_state.my_tsumohai = None
                        for c in consumed_mjai:
                            self.kyoku_state.my_tehai.remove(c)
                ...
                case MSGangType.AddGang:
                    ...
                    if (
                        actor == self.seat
                    ):  # update hand info. kakan is after tsumo, so there is tsumohai
                        self.kyoku_state.my_tehai.append(self.kyoku_state.my_tsumohai)
                        self.kyoku_state.my_tsumohai = None
                        self.kyoku_state.my_tehai.remove(tile_mjai)
                        self.kyoku_state.my_tehai = mj_helper.sort_mjai_tiles(
        ...
        # (3p Mahjong only) LiqiAction.BaBei -> MJAI NUKIDORA
            ...
            if (
                actor == self.seat
            ):  # update hand info. babei is after tsumo, so there is tsumohai
                self.kyoku_state.my_tehai.append(self.kyoku_state.my_tsumohai)
                self.kyoku_state.my_tsumohai = None
            ...
            self.mjai_pending_input_msgs.append(
                {"type": MjaiType.NUKIDORA, "actor": actor, "pai": "N"}
            )
            return self._react_all(liqi_data_data)
    ...
    def _react_all(self, data=None) -> dict | None:
        ...
        if data:
            if (
                ...
                or "operationList" not in data["operation"]
                or len(data["operation"]["operationList"]) == 0
            ):
                return None
        try:
