...
class BotMjai(Bot):
    ...
    def react(self, input_msg: dict) -> dict:
        if self.mjai_bot is None:
            return None
        if (
            self.ignore_next_turn_self_reach
        ):  # ignore repetitive self reach. only for the very next msg
        ...
        if (
            reaction["type"] == MjaiType.REACH and reaction["actor"] == self.seat
        ):  # Self reach
            # get the subsequent dahai message,
            # appeding it to the reach reaction msg as 'reach_dahai' key
            LOGGER.debug(
                "Send reach msg to get reach_dahai. Cannot go back to unreach!"
            )
