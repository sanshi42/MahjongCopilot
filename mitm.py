...
class WSDataInterceptor:
    ...
    def websocket_start(self, flow: HTTPFlow):
        ...
        if self.allow_url(flow.request.pretty_url):
            self.message_queue.put(
                WSMessage(flow.id, flow.timestamp_start, None, WsType.START)
            )
        else:
            flow.kill()
            LOGGER.info(
                "Killing flow since it is not in allowed domains: %s",
    ...
    def websocket_end(self, flow: HTTPFlow):
        ...
        if self.allow_url(flow.request.pretty_url):
            self.message_queue.put(
                WSMessage(flow.id, flow.timestamp_start, None, WsType.END)
            )
...
class MitmController:
    ...
    def start(self, port: int, mode=HTTP, upstream_proxy: str = None):
        ...
        self.mitm_thread = threading.Thread(
            name="MitmThread", target=self._run_mitm_task, daemon=True
        )
    ...
    def install_mitm_cert(self, timeout: float = 5):
        ...
        if install_success:
            ...
            LOGGER.error(
                "Failed to install MITM certificate. Please install manually. Output:\n%s",
                msg,
            )
            return False
